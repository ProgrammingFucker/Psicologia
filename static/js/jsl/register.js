...
document.getElementById('btnloging').addEventListener('click', function () {
    ...
    .then(function (result) {
        ...
        $.post("controller/user.php?op=registro",{usu_nom:result.user.providerData[0].displayName,usu_correo:result.user.providerData[0].email,usu_pass:123456},function(data){
            if(data==0){
                Swal.fire({
                    icon: 'success',
                    title: 'Aceptado!',
                    ...
                    if (result.isConfirmed) {
                        window.open('http://localhost:/login/','_self');
                    }
                });
            }else{
                Swal.fire(
...
document.getElementById('btnloginf').addEventListener('click', function () {
    ...
    .then(function (result) {
        ...
        $.post("registrate.php?op=registro",{usu_nom:result.user.providerData[0].displayName,usu_correo:result.user.providerData[0].email,usu_pass:123456},function(data){
            if(data==0){
                Swal.fire({
                    icon: 'success',
                    title: 'Aceptado!',
                    ...
                    if (result.isConfirmed) {
                        ...
                        window.open('http://localhost/girls/login.php','_self');
                    }
                });
            }else{
                Swal.fire(
