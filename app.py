...
def login():
    ...
    if request.method == 'POST':
        ...
        cursor = mysql.connection.cursor()
        cursor.execute('SELECT * FROM usuario WHERE correo = %s AND password = %s', (email, password))
...
def indexadmin():
    ...
    if request.method == 'POST':
        ...
        cursor = mysql.connection.cursor()
        cursor.execute('SELECT * FROM usuario where correo = %s AND password = %s', (emaila, passworda))
        user = cursor.fetchone()
        if user:
...
def indexpsico():
    ...
    if request.method == 'POST':
        ...
        cursor = mysql.connection.cursor()
        cursor.execute('SELECT * FROM psicologos WHERE correo = %s AND contraseña = %s', (emailp, passwordp))
        user = cursor.fetchone()
        if user:
